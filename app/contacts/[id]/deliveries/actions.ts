...
import { completeDelivery, updateDelivery } from '@/lib/data/deliveries'
...
export async function reopenDelivery(formData: FormData) {
  const supabase = await createClient()
  const {
    data: { user },
    error: authError,
  ...
  await updateDelivery(delivery_id, user.id, {
    completed: false,
    completed_at: null,
  })
